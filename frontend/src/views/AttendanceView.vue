<template>
  <div>
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div>
          ...
          <p class="text-muted" v-if="classInfo">{{ classInfo.name }} – Buổi {{ parseInt(sessionIndex) + 1 }}</p>
        </div>
      </div>
    ...
    <div class="stats-row">
      ...
      <div class="chip chip-gray">📈 Tỉ lệ: {{ attendanceRate }}%</div>
    </div>
    ...
    <div class="card" v-else>
      <div class="card-body" style="padding:0">
        <div v-if="!records.length" class="empty-state"><p>Lớp chưa có sinh viên</p></div>
        <table v-else>
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="(rec, idx) in records" :key="rec.studentId" :class="rowClass(rec.status)">
              <td class="text-muted">{{ idx + 1 }}</td>
              <td><strong>{{ rec.name }}</strong></td>
              <td class="text-muted">{{ rec.code }}</td>
              <td style="text-align:center"><input type="radio" :name="`s-${rec.studentId}`" value="present" v-model="rec.status" /></td>
              <td style="text-align:center"><input type="radio" :name="`s-${rec.studentId}`" value="absent"  v-model="rec.status" /></td>
              <td style="text-align:center"><input type="radio" :name="`s-${rec.studentId}`" value="late"    v-model="rec.status" /></td>
              <td style="text-align:center"><input type="checkbox" v-model="rec.approvedAbsence" :disabled="rec.status === 'present'" /></td>
              <td><input type="text" v-model="rec.note" class="form-control" style="font-size:12px;padding:5px 8px" placeholder="Ghi chú..." /></td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="card" style="margin-top:24px" v-if="summary.length">
      <div class="card-header"><h3>📊 Tổng kết điểm danh cả lớp</h3></div>
      <div class="card-body" style="padding:0">
        <table>
          <thead><tr><th>Sinh viên</th><th>Có mặt</th><th>Vắng</th><th>Trễ</th><th>Tỉ lệ tham gia</th><th>Trạng thái</th></tr></thead>
          <tbody>
            <tr v-for="row in summary" :key="row.student.id">
              <td><div style="font-weight:600">{{ row.student.name }}</div><div class="text-muted">{{ row.student.studentId }}</div></td>
              ...
              <td>
                <span v-if="row.warning" class="badge badge-danger">⚠️ Vắng {{ row.absent }}/{{ row.maxAbsences }} buổi</span>
...
const rowClass = (s) => ({ 'row-present': s==='present', 'row-absent': s==='absent', 'row-late': s==='late' })
...
const saveAttendance = async () => {
  ...
  await attendanceAPI.bulkSave({
    ...
    records: records.value.map(r => ({
      studentId: r.studentId, status: r.status, note: r.note, approvedAbsence: r.approvedAbsence
    }))
  })
  const { data: sum } = await attendanceAPI.getClassSummary(classId)
  summary.value = sum
...
.progress-wrap  { position:relative; background:var(--gray-100); border-radius:99px; height:22px; min-width:120px; overflow:hidden; }
.progress-bar   { height:100%; border-radius:99px; transition:width .4s ease; }
.progress-label { position:absolute; right:8px; top:50%; transform:translateY(-50%); font-size:11px; font-weight:700; color:var(--gray-700); }
</style>
