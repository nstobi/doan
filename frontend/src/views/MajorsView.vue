  ...
  <div>
    ...
    <div class="card">
      <div class="card-body" style="padding:0">
        ...
        <div v-else-if="!majors.length" class="empty-state"><div class="icon">🏫</div><p>Chưa có ngành học nào</p></div>
        <div v-else class="table-wrap">
          <table>
            <thead><tr><th>Tên ngành</th><th>Mã ngành</th><th>Mô tả</th><th>Thao tác</th></tr></thead>
            <tbody>
    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal=false">
      <div class="modal modal-sm">
        ...
        <div class="modal-body">
          ...
          <div class="form-group"><label class="form-label">Tên ngành *</label><input v-model="form.name" class="form-control" placeholder="Công nghệ Thông tin" /></div>
          <div class="form-group"><label class="form-label">Mã ngành *</label><input v-model="form.code" class="form-control" placeholder="CNTT" /></div>
...
const openCreate = () => { editing.value = null; Object.assign(form, { name: '', code: '', description: '' }); error.value = ''; showModal.value = true }
const openEdit   = (m) => { editing.value = m._id; Object.assign(form, { name: m.name, code: m.code, description: m.description }); error.value = ''; showModal.value = true }
const save = async () => {
...
const remove = async (m) => {
  ...
  await majorAPI.remove(m._id); await load()
}
onMounted(load)
</script>
