    ...
    <div v-else-if="assignment">
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-header">
          <div><h3>{{ assignment.title }}</h3><span class="badge" :class="typeBadge(assignment.type)">{{ typeLabel(assignment.type) }}</span></div>
          <span :style="{ color: isOverdue ? 'var(--danger)' : 'var(--gray-500)', fontWeight: isOverdue ? '600' : '400' }">⏰ Hạn nộp: {{ formatDateTime(assignment.dueDate) }}</span>
        </div>
        <div class="card-body">
      ...
      <!-- Already submitted -->
      <div class="card" style="margin-bottom:20px" v-if="existingSubmission">
        <div class="card-header">
          <h3>✅ Bài đã nộp</h3>
          <span class="badge" :class="subStatusBadge(existingSubmission.status)">{{ subStatusLabel(existingSubmission.status) }}</span>
        </div>
        <div class="card-body">
          <div v-if="existingSubmission.score !== null && existingSubmission.score !== undefined" class="alert alert-success">
            🎯 Điểm của bạn: <strong>{{ existingSubmission.score }}/{{ assignment.maxScore }}</strong>
            <div v-if="existingSubmission.feedback" style="margin-top:8px">💬 Nhận xét: {{ existingSubmission.feedback }}</div>
          </div>
          <div v-else class="alert alert-info">Bài đã nộp lúc {{ formatDateTime(existingSubmission.createdAt) }}. Chờ giáo viên chấm điểm.</div>
          <button class="btn btn-ghost btn-sm" style="margin-top:8px" @click="existingSubmission = null">🔄 Nộp lại bài</button>
        </div>
      </div>
      ...
      <div class="card" v-else>
        ...
        <div class="card-body">
          <div v-if="isOverdue" class="alert alert-error">⚠️ Đã quá hạn nộp! Bài nộp sẽ được đánh dấu là trễ.</div>
          ...
          <div class="form-group">
            ...
            <select v-model="form.studentId" class="form-control">
              ...
              <option v-for="s in students" :key="s._id" :value="s._id">{{ s.studentId }} — {{ s.name }}</option>
            </select>
          </div>

          <div class="form-group">
            <label class="form-label">Nội dung bài làm</label>
            <textarea v-model="form.content" class="form-control" rows="8" placeholder="Nhập câu trả lời hoặc nội dung bài làm..."></textarea>
          </div>

          <div class="form-group">
            <label class="form-label">File đính kèm</label>
            <input type="file" multiple @change="onFiles" class="form-control" />
            <div class="text-muted" style="margin-top:4px">Có thể đính kèm nhiều file (PDF, Word, ảnh...)</div>
          </div>
        ...
        <div class="modal-footer" style="border-top:1px solid var(--gray-200)">
          <button class="btn btn-primary" @click="submitWork" :disabled="submitting">{{ submitting ? 'Đang nộp...' : '📤 Nộp bài' }}</button>
        </div>
      </div>
    </div>
  </div>
...
const isOverdue      = computed(() => assignment.value ? new Date() > new Date(assignment.value.dueDate) : false)
...
const submitWork = async () => {
  ...
  if (!form.content && !selectedFiles.value.length) return (error.value = 'Vui lòng nhập nội dung hoặc đính kèm file')
...
.file-chip   { display:inline-flex; align-items:center; gap:6px; padding:4px 10px; background:var(--gray-100); border-radius:99px; font-size:12px; margin:4px; }
</style>
