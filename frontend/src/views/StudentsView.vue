  ...
  <div>
    ...
    <div class="card">
      <div class="card-body" style="padding:0">
        ...
        <div v-else-if="!students.length" class="empty-state"><div class="icon">👩‍🎓</div><p>Không tìm thấy sinh viên</p></div>
        <div v-else class="table-wrap">
          <table>
            <thead><tr><th>Sinh viên</th><th>Ngành</th><th>Kỳ</th><th>Số môn</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
              <tr v-for="s in students" :key="s._id">
                <td><div style="font-weight:600">{{ s.name }}</div><div class="text-muted">{{ s.studentId }} · {{ s.email }}</div></td>
                <td>{{ s.major?.name || '—' }}</td>
    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal=false">
      <div class="modal modal-md">
        ...
        <div class="modal-body">
          ...
          <div v-if="!editing" class="alert alert-info">💡 Sinh viên sẽ được <strong>tự động đăng ký môn học kỳ 1</strong> của ngành được chọn.</div>
          <div class="grid-2">
            <div class="form-group"><label class="form-label">Mã sinh viên *</label><input v-model="form.studentId" class="form-control" placeholder="SV001" /></div>
            <div class="form-group"><label class="form-label">Họ và tên *</label><input v-model="form.name" class="form-control" /></div>
          </div>
          <div class="grid-2">
            <div class="form-group"><label class="form-label">Email *</label><input v-model="form.email" type="email" class="form-control" /></div>
            <div class="form-group"><label class="form-label">Điện thoại</label><input v-model="form.phone" class="form-control" /></div>
          </div>
          <div class="grid-2">
            ...
            <div class="form-group"><label class="form-label">Ngày sinh</label><input v-model="form.dateOfBirth" type="date" class="form-control" /></div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="showModal=false">Hủy</button>
          <button class="btn btn-primary" @click="save" :disabled="saving">{{ saving ? 'Đang lưu...' : (editing ? 'Cập nhật' : 'Tạo & Tự động đăng ký') }}</button>
        </div>
      </div>
    </div>
  </div>
...
const openCreate = () => { editing.value=null; Object.assign(form,{studentId:'',name:'',email:'',phone:'',major:'',dateOfBirth:''}); error.value=''; showModal.value=true }
const openEdit   = (s) => { editing.value=s._id; Object.assign(form,{studentId:s.studentId,name:s.name,email:s.email,phone:s.phone,major:s.major?._id||'',dateOfBirth:s.dateOfBirth?.split('T')[0]||''}); error.value=''; showModal.value=true }
...
const remove = async (s) => { if (!confirm(`Xóa sinh viên "${s.name}"?`)) return; await studentAPI.remove(s._id); await load() }
