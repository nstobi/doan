<template>
  <div>
    ...
    <div v-else-if="cls">
            ...
            <table>
              <tbody>
                <tr v-for="s in cls.students" :key="s._id">
                  <td><div style="font-weight:500">{{ s.name }}</div><div class="text-muted">{{ s.studentId }}</div></td>
                </tr>
                <tr v-if="!cls.students?.length"><td class="empty-state">Chưa có sinh viên</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      <!-- Tabs -->
      <div class="tabs">
        <button class="tab" :class="{ active: activeTab === 'sessions' }" @click="activeTab = 'sessions'">📅 Buổi học</button>
        <button class="tab" :class="{ active: activeTab === 'materials' }" @click="activeTab = 'materials'; loadMaterials()">📁 Học liệu</button>
        <button class="tab" :class="{ active: activeTab === 'assignments' }" @click="activeTab = 'assignments'; loadAssignments()">📝 Bài thi</button>
      </div>
      ...
      <div class="card" v-if="activeTab === 'sessions'">
        ...
        <div class="card-body" style="padding:0">
          <div v-if="!cls.sessions?.length" class="empty-state"><div class="icon">📅</div><p>Chưa có buổi học. Thêm buổi học để bắt đầu điểm danh!</p></div>
          <table v-else>
            <thead><tr><th>Buổi</th><th>Ngày</th><th>Nội dung</th><th>Thao tác</th></tr></thead>
            <tbody>
              <tr v-for="(session, idx) in cls.sessions" :key="idx">
                ...
                <td>
                  <router-link :to="`/attendance/${cls._id}/session/${idx}`" class="btn btn-primary btn-sm">📝 Điểm danh</router-link>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="card" v-if="activeTab === 'materials'">
        <div class="card-header">
          ...
          <button class="btn btn-primary btn-sm" @click="showUpload = true">+ Upload tài liệu</button>
        </div>
        <div class="card-body" style="padding:0">
          <div v-if="loadingMaterials" class="loading-wrap"><div class="spinner"></div></div>
          <div v-else-if="!materials.length" class="empty-state"><div class="icon">📁</div><p>Chưa có học liệu nào</p></div>
          <table v-else>
            <thead><tr><th>Tài liệu</th><th>Loại</th><th>Kích thước</th><th>Người upload</th><th>Thao tác</th></tr></thead>
            <tbody>
              <tr v-for="m in materials" :key="m._id">
                <td><div style="font-weight:600">{{ m.title }}</div><div class="text-muted">{{ m.originalName }}</div></td>
                <td><span class="badge" :class="typeBadge(m.type)">{{ typeIcon(m.type) }} {{ m.type }}</span></td>
      ...
      <div class="card" v-if="activeTab === 'assignments'">
        <div class="card-header">
          ...
          <button class="btn btn-primary btn-sm" @click="showCreateAssignment = true">+ Tạo bài thi</button>
        </div>
        <div class="card-body" style="padding:0">
          <div v-if="loadingAssignments" class="loading-wrap"><div class="spinner"></div></div>
          <div v-else-if="!assignments.length" class="empty-state"><div class="icon">📝</div><p>Chưa có bài thi nào</p></div>
          <table v-else>
            <thead><tr><th>Tiêu đề</th><th>Loại</th><th>Hạn nộp</th><th>Điểm tối đa</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
              <tr v-for="a in assignments" :key="a._id">
                <td><div style="font-weight:600">{{ a.title }}</div><div class="text-muted">{{ a.description }}</div></td>
                <td><span class="badge" :class="assignTypeBadge(a.type)">{{ assignTypeLabel(a.type) }}</span></td>
                <td :style="{ color: isOverdue(a.dueDate) ? 'var(--danger)' : '' }">
                ...
                <td>
                  <div class="flex gap-2">
                    <router-link :to="`/assignments/${a._id}`" class="btn btn-primary btn-sm">👁 Chi tiết</router-link>
          ...
          <div style="padding:16px;border-top:1px solid var(--gray-200)" v-if="assignments.length">
            ...
            <div v-for="a in assignments" :key="'link-'+a._id" style="margin-bottom:6px">
              <router-link :to="`/assignments/${a._id}/submit`" class="btn btn-ghost btn-sm">📤 Nộp bài: {{ a.title }}</router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="modal-overlay" v-if="showSessionModal" @click.self="showSessionModal=false">
      <div class="modal modal-sm">
        <div class="modal-header"><h3>Thêm buổi học</h3><button class="btn btn-ghost btn-sm btn-icon" @click="showSessionModal=false">✕</button></div>
        <div class="modal-body">
          <div class="form-group"><label class="form-label">Ngày học *</label><input v-model="sessionForm.date" type="date" class="form-control" /></div>
          <div class="form-group"><label class="form-label">Nội dung</label><input v-model="sessionForm.description" class="form-control" placeholder="Buổi 1: Giới thiệu môn học" /></div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="showSessionModal=false">Hủy</button>
          <button class="btn btn-primary" @click="addSession" :disabled="savingSession">{{ savingSession ? 'Đang lưu...' : 'Thêm buổi học' }}</button>
        </div>
      </div>
    </div>
    ...
    <div class="modal-overlay" v-if="showUpload" @click.self="showUpload=false">
      <div class="modal modal-sm">
        <div class="modal-header"><h3>Upload Học liệu</h3><button class="btn btn-ghost btn-sm btn-icon" @click="showUpload=false">✕</button></div>
        <div class="modal-body">
          <div v-if="uploadError" class="alert alert-error">{{ uploadError }}</div>
          <div class="form-group"><label class="form-label">Tiêu đề</label><input v-model="uploadForm.title" class="form-control" placeholder="Bài giảng tuần 1" /></div>
          <div class="form-group"><label class="form-label">Mô tả</label><input v-model="uploadForm.description" class="form-control" /></div>
          <div class="form-group">
            <label class="form-label">Chọn file *</label>
            <input type="file" @change="onFileChange" class="form-control" />
            <div class="text-muted" style="margin-top:4px">PDF, Word, PowerPoint, ảnh, video (tối đa 100MB)</div>
          </div>
          <div v-if="uploading" style="margin-top:12px">
            <div class="text-muted" style="margin-bottom:6px">Đang upload... {{ uploadProgress }}%</div>
            <div class="progress-wrap"><div class="progress-bar-upload" :style="{ width: uploadProgress + '%' }"></div></div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="showUpload=false">Hủy</button>
          <button class="btn btn-primary" @click="uploadFile" :disabled="uploading || !selectedFile">{{ uploading ? 'Đang upload...' : '⬆️ Upload' }}</button>
        </div>
      </div>
    </div>
    ...
    <div class="modal-overlay" v-if="showCreateAssignment" @click.self="showCreateAssignment=false">
      <div class="modal modal-md">
        <div class="modal-header"><h3>Tạo bài thi / bài tập</h3><button class="btn btn-ghost btn-sm btn-icon" @click="showCreateAssignment=false">✕</button></div>
        <div class="modal-body">
          <div v-if="assignmentError" class="alert alert-error">{{ assignmentError }}</div>
          <div class="form-group"><label class="form-label">Tiêu đề *</label><input v-model="assignmentForm.title" class="form-control" placeholder="Bài kiểm tra giữa kỳ" /></div>
          <div class="form-group"><label class="form-label">Mô tả / Đề bài</label><textarea v-model="assignmentForm.description" class="form-control" rows="4" placeholder="Nội dung bài thi..."></textarea></div>
          <div class="grid-2">
            <div class="form-group">
            ...
            <div class="form-group"><label class="form-label">Điểm tối đa</label><input v-model.number="assignmentForm.maxScore" type="number" min="1" max="100" class="form-control" /></div>
          </div>
          <div class="form-group"><label class="form-label">Hạn nộp *</label><input v-model="assignmentForm.dueDate" type="datetime-local" class="form-control" /></div>
          <div class="form-group"><label class="form-label">File đề bài (không bắt buộc)</label><input type="file" multiple @change="onAssignmentFiles" class="form-control" /></div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="showCreateAssignment=false">Hủy</button>
          <button class="btn btn-primary" @click="createAssignment" :disabled="savingAssignment">{{ savingAssignment ? 'Đang tạo...' : 'Tạo bài thi' }}</button>
        </div>
      </div>
    </div>
  </div>
...
const assignmentForm       = reactive({ title: '', description: '', type: 'assignment', maxScore: 10, dueDate: '' })
...
const assignTypeBadge = (t) => ({ assignment:'badge-info', exam:'badge-danger', quiz:'badge-warning' }[t])
const assignTypeLabel = (t) => ({ assignment:'📋 Bài tập', exam:'📝 Bài thi', quiz:'⚡ Kiểm tra nhanh' }[t])
...
const onFileChange = (e) => {
  ...
  if (selectedFile.value && !uploadForm.title) uploadForm.title = selectedFile.value.name
}
...
const uploadFile = async () => {
  ...
  try {
    ...
    uploadForm.title = ''; uploadForm.description = ''; selectedFile.value = null
    await loadMaterials()
  } catch(e) { uploadError.value = e.response?.data?.message || 'Upload thất bại' }
...
const removeMaterial = async (m) => {
  ...
  await materialAPI.remove(m._id); await loadMaterials()
}

const onAssignmentFiles = (e) => { assignmentFiles.value = Array.from(e.target.files) }

const createAssignment = async () => {
  if (!assignmentForm.title || !assignmentForm.dueDate) return (assignmentError.value = 'Vui lòng điền tiêu đề và hạn nộp')
  savingAssignment.value = true; assignmentError.value = ''
  try {
    const formData = new FormData()
    ...
    assignmentForm.title = ''; assignmentForm.description = ''; assignmentForm.dueDate = ''
    await loadAssignments()
  } catch(e) { assignmentError.value = e.response?.data?.message || 'Có lỗi xảy ra' }
  finally { savingAssignment.value = false }
}

const removeAssignment = async (a) => {
  if (!confirm(`Xóa bài thi "${a.title}"?`)) return
  await assignmentAPI.remove(a._id); await loadAssignments()
}
