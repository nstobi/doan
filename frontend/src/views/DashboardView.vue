...
onMounted(async () => {
  try {
    const [majors, subjects, students, classes] = await Promise.all([
      majorAPI.getAll(), subjectAPI.getAll(), studentAPI.getAll(), classAPI.getAll()
    ])
...
<style scoped>
.stat-card  { background: var(--white); border-radius: var(--radius); border: 1px solid var(--gray-200); padding: 20px; display: flex; align-items: center; gap: 16px; box-shadow: var(--shadow-sm); }
